...
"""Bring a box to a target and orientation with the reBot Arm B601-DM."""
...
def default_config() -> config_dict.ConfigDict:
  """Returns the default config for reBot B601-DM pick-cube tasks."""
  config = config_dict.create(
      ...
      action_repeat=1,
      action_scale=0.04,
      # Half-extents (x, y) of the box spawn area around the keyframe box.
      box_init_range=[0.07, 0.12],
      # Half-extents (x, y) of the target area around the keyframe box, and the
      # target height range above it. joint4 limits keeping the gripper pointing
      # down above ~0.18 m, so targets are lower than in PandaPickCube.
      target_xy_range=[0.07, 0.12],
      target_z_range=[0.08, 0.15],
...
class RebotDmPickCube(base.RebotDmBase):
  ...
  def reset(self, rng: jax.Array) -> State:
    ...
    box_x, box_y = self._config.box_init_range
    box_pos = (
        jax.random.uniform(
            rng_box,
            (3,),
            minval=jp.array([-box_x, -box_y, 0.0]),
            maxval=jp.array([box_x, box_y, 0.0]),
        )
        + self._init_obj_pos
    )

    # initialize target position
    target_x, target_y = self._config.target_xy_range
    target_z_min, target_z_max = self._config.target_z_range
    target_pos = (
        jax.random.uniform(
            rng_target,
            (3,),
            minval=jp.array([-target_x, -target_y, target_z_min]),
            maxval=jp.array([target_x, target_y, target_z_max]),
        )
        + self._init_obj_pos
    )
...
class RebotDmPickCubeOrientation(RebotDmPickCube):
